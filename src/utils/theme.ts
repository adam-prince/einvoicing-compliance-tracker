...
function toColor(v: unknown): string | undefined {
...
export function applyThemeFromJson(theme: any) {
	...
	for (const [cssVar, keys] of Object.entries(candidates)) {
		...
		for (const k of keys) {
			...
			if (val) { setCssVar(cssVar, val); applied = true; break; }
		}
		if (!applied) {
			const fallback = findFirstColor(theme, cssVar);
			if (fallback) setCssVar(cssVar, fallback);
		}
	}
	// Typography
...
function findFirstColor(obj: any, hint: string): string | undefined {
	try {
		const queue: any[] = [obj];
		while (queue.length) {
			...
			if (cur && typeof cur === 'object') {
				for (const [k, v] of Object.entries(cur)) {
					if (typeof v === 'string' && isColorString(v)) return v;
