...
import { dataSources, type DataSource } from '../data/formatSpecifications';

export interface TimelineEvent {
	date: string;
	description: string;
	threshold?: string;
	status: 'mandated' | 'planned' | 'permitted';
	category: 'B2G' | 'B2B' | 'B2C' | 'reporting';
}
...
const enhancedComplianceDatabase: Record<string, EnhancedComplianceData> = {
	'DEU': {
		countryId: 'DEU',
		countryName: 'Germany',
		timeline: [
...
export class ComplianceDataService {
	private static instance: ComplianceDataService;
	private cache: Map<string, EnhancedComplianceData> = new Map();
	private lastRefresh: Date | null = null;
	private dataSourcesLastUpdated: Map<string, Date> = new Map();

	private constructor() {}

	public static getInstance(): ComplianceDataService {
	...
	public async refreshComplianceData(
		countryId?: string,
		onProgress?: (progress: ProgressUpdate) => void
	): Promise<EnhancedComplianceData[]> {
		const results: EnhancedComplianceData[] = [];
	...
	private async updateDataSourceTimestamps(): Promise<void> {
		const now = new Date();
	...
	public getComplianceTimeline(countryId: string): EnhancedComplianceData | null {
		// Check cache first
	...
	public generateSampleTimeline(countryName: string, isoCode: string): EnhancedComplianceData {
	...
	public needsDataSourceUpdate(): boolean {
	...
	public getComplianceSummary(): {
		...
		return {
			...
			mandatedB2G: countries.filter(c => 
				c.timeline.some(t => t.category === 'B2G' && t.status === 'mandated')
			).length,
			plannedB2B: countries.filter(c => 
				c.timeline.some(t => t.category === 'B2B' && t.status === 'planned')
			).length,
